...
class Gc_dependent_select
{
        ...
        if (!empty($this->fields)) {
            foreach ($this->fields as $k => $v) {
                if (!empty($v['relate'])) {
                    ...
                    }elseif($v['field_type']=='input'){
                        $this->grocery_crud_obj->callback_edit_field($k, array($this, 'callback_edit_input'));
                        $this->grocery_crud_obj->callback_add_field($k, array($this, 'callback_add_input'));
                    }else{
                        $this->grocery_crud_obj->callback_edit_field($k, array($this, 'callback_edit'));
                        $this->grocery_crud_obj->callback_add_field($k, array($this, 'callback_add'));
                    }
    ...
    function callback_edit($value, $primary_key, $field_info)
    {
        ...
        $table_name = $this->fields[$field_info->name]['table_name']; // additional script

        // $this->ci->db->select("*")->from($field_info->extras[1])->where($where);
        $this->ci->db->select("*")->from($table_name)->where($where); // edited script
    ...
    function callback_edit_input($value, $primary_key, $field_info)
    {
        $i = 0;
        $f_array = array();
        $parent = array();
        ...
        $html = $this->get_input_text_html($result, $value, $field_info);
        return $html;
    }
    ...
    function get_input_text_html($result, $value, $field_info)
    {
        foreach ($result as $item) {
            $val = $item[$this->fields[$field_info->name]['id_field']];
        }

        $html = '<input type="text" name="'. $field_info->name . '" class="form-control" data-placeholder="' . $this->fields[$field_info->name]['data-placeholder'] . '" value="'.$val.'">';
    ...
    function callback_add_input($value, $primary_key, $field_info)
    {
        print_r($value);
        $html = '<input type="text" disabled="disabled" name="'. $field_info->name . '" class="form-control" data-placeholder="' . $this->fields[$field_info->name]['data-placeholder'] . '" value="'.$value.'">';
        return $html;
    }
    ...
    function get_js()
    {
        ...
        $i = 0;
        foreach ($this->fields as $k => $v) {
        	if(isset($this->fields[$k]['field_type']) && $this->fields[$k]['field_type']=='input')
        	{
        		$js .= 'var ' . $k . ' = $(\'input[name="' . $k . '"]\');';
        ...
        for ($i = 1; $i <= sizeof($dd_dropdowns) - 1; $i++) {
            ...
            if(isset($this->fields[$dd_dropdowns[$i]['field']]['field_type']) && $this->fields[$dd_dropdowns[$i]['field']]['field_type']=='input')
            {
		        $js .= 'if(data==\'\'){';
		        $js .= $dd_dropdowns[$i]['field'] . '.val("");';
		        $js .= $dd_dropdowns[$i]['field'] . '.attr(\'disabled\', true);';
		        $js .= '}else{';
		        $js .= '$.each(data, function(key, val) {';
		        $js .= $dd_dropdowns[$i]['field'] . '.val(val.value);';
		        $js .= '});';
		        $js .= '}';
		        if (!empty($this->config['ajax_loader'])) {
		        ...
		        for ($x = $i + 1; $x <= sizeof($dd_dropdowns) - 1; $x++) {
			        if(isset($this->fields[$dd_dropdowns[$x]['field']]['field_type']) && $this->fields[$dd_dropdowns[$x]['field']]['field_type']=='input')
	            	{
				        $js .= $dd_dropdowns[$x]['field'] . '.val("");';
	            	}else{
